...
const classeSelect = "form-control";
let carrello = [];
let panini = [];
scuole = [];
sedi = [];
...
function login() {
    ...
    if (nome.length === 0 || password.length === 0) {
        alert("Non hai inserito la password o l'username");
        return;
    }
    $.ajax({
        ...
        success: function (data) {
            data = data.replace("\n", "");
            data = data.replace("\r", "");
            if (data === "alert('Yo hai missato qualcosa')") {
...
function ottieniScuola() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            selectTag.className = classeSelect;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (let i = 0; i < panini.length; i++) {
                ...
                tagImmagine.style.display = "block;";
...
function stampaTabellaPaninara(ordine) {
    ...
    for (let i = 0; i < classi.length; i++) {
        ...
        let testoTitolo = document.createElement("h5");
        testoTitolo.innerHTML = "<br><br>La classe " + classe.nomeClasse + " ha ordinato " + classe.calcolaNumeroPaniniOrdinati() + " per il costo di " + classe.calcolaPrezzoTotale() + "€";

        titolo.append(testoTitolo);

        titolo.className = "table mx-auto";

        let tabella = document.createElement("table");
        tabella.className = "table mx-auto";
        tabella.style.width = "800px";
        tabella.border = 2;

        let testaTabella = document.createElement("thead");
        let rigaTesta = document.createElement("tr");

        let nomePaninoTesta = document.createElement("th");
        nomePaninoTesta.textContent = "Nome del panino";
        nomePaninoTesta.scope = "col"


        let prezzoTesta = document.createElement("th");
        prezzoTesta.scope = "col"
        prezzoTesta.textContent = "Prezzo"


        let quantitaTesta = document.createElement("th");
        quantitaTesta.textContent = "Quantita"
        quantitaTesta.scope = "col"

        let compraTesta = document.createElement("th");
        compraTesta.textContent = "Compra"
        compraTesta.scope = "col"


        rigaTesta.appendChild(nomePaninoTesta);
        rigaTesta.appendChild(prezzoTesta);
        rigaTesta.appendChild(quantitaTesta);
        rigaTesta.appendChild(compraTesta);

        testaTabella.append(rigaTesta)

        tabella.append(testaTabella);
        ...
        for (let j = 0; j < classe.panini.length; j++) {
            ...
            pulsante.className = "btn btn-primary mx-auto";
...
function parsaScuolaSedeClasse() {
    ...
    selectScuola.className = classeSelect;
